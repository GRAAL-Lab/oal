...
oal::AStarNode::AStarNode(EncounterData d, std::shared_ptr<AStarNode> parent, bool goal)
    : data(d)
    , isGoal(goal)
{
    if (parent != nullptr) {
        if (parent->costToReach == -1) {
            parent->Print();
            std::cerr << "[WARNING] Linked to node with no costs set\n";
        }
        parent_ = parent;
...
{
    if (abs(data.time.count()) > 0.01)
        throw std::runtime_error("[GetExitVxs] Time should be 0 for this function");
    auto obs = *data.obs_ptr;
    auto obs_P_vehicle = obs.InitialPose().FromWorld2ThisFrame(data.position);
    auto vxs = obs.LocalVxs();

    auto x = [vxs](VxId id) { return vxs[id].second.x(); };
    auto y = [vxs](VxId id) { return vxs[id].second.y(); };

    bool IsLeftOfDiag1 = (obs_P_vehicle.y() >= y(FL) / x(FL) * obs_P_vehicle.x()); // Being left of diag FL-RR
    bool IsLeftOfDiag2 = (obs_P_vehicle.y() >= y(FR) / x(FR) * obs_P_vehicle.x()); // Being left of diag FR-LL
...
void oal::AStarNode::SetCosts(const VehicleData& vh_data, Eigen::Vector2d goal)
{
    ...
    if (vh_data.velocities.empty()) {
        ...
        throw std::runtime_error("velocities vector is empty");
    } else {
        auto max_it = std::max_element(vh_data.velocities.begin(), vh_data.velocities.end());
        double max_vel = *max_it;
        costToGoal = dist_to_goal.norm() / max_vel;
    }
