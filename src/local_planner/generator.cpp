...
oal::PathReport oal::Generator::FindPath(const Pose& start_, Eigen::Vector2d goal_, const std::vector<ObsPtr>& obstacles_, AstarPath& path_, bool colregs)
{
    ...
    if (!GetFirstNodes(start_, goal_, obstacles_, openSet, response))
        return response;
    ...
    while (!openSet.empty()) {
        ...
        if (pruning_params_.timeOut > 0 && time_result > pruning_params_.timeOut) { // time failure limit
            timeout = true;
            break;
        }
        ...
        // check final
        if (current->isGoal || (current->data.position - goal_).norm() < pruning_params_.samePositionThreshold) {
            break;
        }
    ...
    if (current->isGoal) {
        if ((current->data.position - goal_).norm() < pruning_params_.samePositionThreshold) {
            ...
            double distFromStart = (current->data.position - start_.Position()).norm();
            bool existsPath = distFromStart > pruning_params_.samePositionThreshold;
            response.result = existsPath ? SearchResult::PARTIAL : SearchResult::FAIL;
            response.failMsg = existsPath ? "Goal is in bb, going just outside of it." : "Goal is in bb and best candidate is too close to start";
        }
    ...
    auto best_candidate = *GetBestFromSet(reachableSet);
    double distFromStart = (best_candidate->data.position - start_.Position()).norm();
    bool existsPath = distFromStart > pruning_params_.samePositionThreshold;

    if (timeout) {
        response.result = existsPath ? SearchResult::PARTIAL : SearchResult::FAIL;
        response.failMsg = existsPath ? "Timeout but getting close to target." : "Timeout and no path found";
    } else {
        response.result = existsPath ? SearchResult::PARTIAL : SearchResult::FAIL;
        response.failMsg = existsPath ? "Goal unreachable but getting close to target." : "Goal unreachable and best candidate is too close to start";
    }
...
bool oal::Generator::IsPathValid(AstarPath path, const Pose& vehicle, const std::vector<ObsPtr>& obstacles_, Eigen::Vector2d& unreachable_wp, const ObsPtr& startingObs, const VxId& startingVx)
{
    ...
    start_data.obs_ptr = startingObs;
    start_data.vx = startingVx;
    ...
    while (!path.Data().empty()) {
        ...
        for (const auto& obs : obstacles_) {
            ...
            bool colregs = false;
            if (PathEvaluator::CollisionWithObs(start, target, obs, collisions, checkingPath, colregs)) {

                Eigen::Vector3d closest_collision;
                double min_dist = std::numeric_limits<double>::max();
                for (const auto& collision : collisions) {
                    double dist = (collision.head(2) - start->data.position).norm();
                    if (dist < min_dist) {
                        min_dist = dist;
                        closest_collision = collision;
                    }
                }
                EncounterData collision_data;
                collision_data.position = closest_collision.head(2);
                collision_data.time = std::chrono::duration<double>(closest_collision.z());
                collision_data.approachingSpeed = target->data.approachingSpeed;
                collision_data.obs_ptr = obs;
                start->SetCosts(vh_data_, target->data.position); // useless, just to avoid warning about costs not being set
                auto collision_node = std::make_shared<AStarNode>(collision_data, start);
                if (ds_->notValidPathNodesLog.log)
                    collision_node->Log(logNodesNVPFile);
                unreachable_wp = target->data.position;
                if (ds_->printPathUnsafetyReason)
                    std::cerr << "Cannot reach wp: " << unreachable_wp.transpose() << std::endl;

                if (ds_->logObstacles)
                    LogObstacles(obstacles_, "notValidPathObstacles");
                return false;
            }
        }
        ...
        if (path.Data().empty())
            if (ds_->validPathNodesLog.log)
                target->Log(logNodesVPFile);
    }
    return true;
}
...
bool oal::Generator::GetNodesToGoal(const NodePtr& current, const Eigen::Vector2d& goal_, const std::vector<ObsPtr>& obstacles_, NodeSet& successors)
{
    ...
    for (const auto& speed : vh_data_.velocities) {
        ...
        if (IsInBB(data.time, goal_, obstacles_, so)) {
            // TODO check if this is the best way to handle this case
            if (!pruning_params_.stopSearchIfGoalInBB)
            ...
            for (const auto& obs : so) {
                ...
                for (const auto& c : collisions) {
                    ...
                    //     data.obs_ptr = obs;
                    // }
                    if ((c.head(2) - current->data.position).norm() < (actual_goal - current->data.position).norm()) {
                        actual_goal = c.head(2);
                        data.obs_ptr = obs;
                    }
                }
    ...
    NodeSet& closedSet, const NodeSet& openSet)
{
    ...
    for (NodePtr& tentative_node : successors) {
        ...
        if (pruning_params_.colregsCompliant && tentative_node->data.obs_ptr && !PathEvaluator::RuleCompliantMotion(current, tentative_node)) {
    ...
    std::vector<ObsPtr>& surrounding_obs)
{
    ...
    for (const auto& obs : obstacles) {
        ...
        // std::cerr << "Obs: " << obs->Id() << "\n vx1: " << vxs[0].second.transpose() << "\n vx2: " << vxs[1].second.transpose() << "\n vx3: " << vxs[2].second.transpose() << "\n vx4: " << vxs[3].second.transpose() << "\n";
        //  re-order to form a polygon
        std::vector<Eigen::Vector2d> vxs_positions = {
            vxs[1].second, // Top left (-14.5, 28.5)
            vxs[0].second, // Top right (18.5, 28.5)
            vxs[2].second, // Bottom right (18.5, -4.5)
            vxs[3].second // Bottom left (-14.5, -4.5)
        };
...
{
    for (auto& n : nodes) {
        bool isSimilar = ((n->data.position - node->data.position).norm() < pruning_params_.samePositionThreshold && std::abs((n->data.time - node->data.time).count()) < pruning_params_.sameTimeThreshold);
...
void oal::Generator::LogObstacles(const std::vector<ObsPtr>& obstacles, std::string description)
{
    for (const auto& obs : obstacles) {
        obs->Log(logObstaclesFile, description);
    }
}
