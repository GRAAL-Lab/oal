  ...
  */
  ...
  for (const obs_ptr &obs_ptr: obss_info_.obstacles) {
    ...
    if (colregs_compliance) {
      ...
      if (theta > HeadOnAngle && theta < OvertakingAngle && !obs_ptr->higher_priority && obs_ptr->speed>OBSTACLE_STATIC_SPEED_THRESHOLD) {
    ...
    std::vector<std::vector<int>> side_idxs = {{0, 2}, {0, 1}, {3, 2}, {3,1}};
    int vx_idx1, vx_idx2;
