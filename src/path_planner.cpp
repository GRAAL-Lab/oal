...
*/
        ...
        for (const obs_ptr &obs_ptr: obss_info_.obstacles) {
            ...
            for (double &speed: v_info_.velocities) {
                ...
                average_time.find_points = (average_time.find_points * average_time.n_points + elapsed_seconds.count() ) / (average_time.n_points+1);
                average_time.n_points++;
            }
            ...
            for (const Vertex &vx: reachable_vxs) {
                ...
                //if(current.vx == RL && current.obs_ptr->id == "2" && new_node.vx == RR  && new_node.obs_ptr->id == "1" ) std::cout<<"trye\n";

                Node similar;
/*                if (new_node.HasSimilarIn(close_set, similar)) {
                    if(current.vx == RL && current.obs_ptr->id == "2" && new_node.vx == RR  && new_node.obs_ptr->id == "1" ) {
                        std::cout << "1\n";
                    }
                    continue;
                }*/
                if (new_node.HasSimilarIn(reachable_full_set, similar)) {
                    //if(current.vx == RL && current.obs_ptr->id == "2" && new_node.vx == RR  && new_node.obs_ptr->id == "1" ) std::cout<<"2\n";
                    continue;
                }
                if (!new_node.IsUnique(current) || !CheckColreg(current, new_node) ||
                    !CheckCollision(current, new_node)) {
                    // Not unique means it is returning on a vertex already visited.
                    close_set.insert(new_node);
                    //if(current.vx == RL && current.obs_ptr->id == "2" && new_node.vx == RR  && new_node.obs_ptr->id == "1" ) std::cout<<"3\n";
                    continue;
                }
                ...
                average_time.set_order = (average_time.set_order * average_time.set_ordered + elapsed_seconds.count() ) / (average_time.set_ordered+1);
                average_time.set_ordered++;
...
bool path_planner::CheckCollision(const Node &start, Node &goal,
    ...
    */
    ...
    // Plot stuff
    {
        ...
        if(start.vx == RL && start.obs_ptr->id == "2" && goal.vx == RR  && goal.obs_ptr->id == "1" ) plotCKFile_ << "Good" << std::endl;
    }
    ...
    for (const obs_ptr &obs_ptr: obss_info_.obstacles) {
        ...
        // Plot stuff
        {
            ...
            plotCKFile_ << "Direction_" << bb_direction(0) << "_" << bb_direction(1) << "_" << bb_direction(2)
                        << std::endl;
        ...
        if (isDepartingObs && start.vx == NA) {
            if (!isApproachingObs) {
                ...
                if (FindLinePlaneIntersectionPoint(vxs_abs[FR], vxs_abs[RL], bb_direction, start, goal, collision_point)
                    || FindLinePlaneIntersectionPoint(vxs_abs[FL], vxs_abs[RR], bb_direction, start, goal,
                                                      collision_point)) {
                    return false;
                }
            }
    ...
    average_time.check_collision = (average_time.check_collision * average_time.n_ck + elapsed_seconds.count() ) / (average_time.n_ck+1);
    average_time.n_ck++;
    return true;
}
    ...
    for (auto i = 0; i < 4; i++) {
        if (visible_vxs[i].isVisible) {
            ...
            if (obstacle.speed < 0.001) { // TODO choose value
                ...
                double c1 =
                        -(bb_timeDirection.x() * vertex_vehicle.x() + bb_timeDirection.y() * vertex_vehicle.y()) / det;
...
bool path_planner::CheckPath(const Eigen::Vector2d &vh_pos, Path path, Eigen::Vector2d &unreachable_wp) {
    ...
    Path whole_path = path;// to print later

    path.pop();

    //colregs_compliance = false; // should not change the value, otherwise ignoring obs (giving way) would be threat
    while (!path.empty()) {
        Node goal = path.top();
