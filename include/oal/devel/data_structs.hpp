...
public:
    ...
    Eigen::Vector2d FromThis2WorldFrame(Eigen::Vector2d local_P) const {
    ...
    Eigen::Vector2d FromWorld2ThisFrame(Eigen::Vector2d world_P) const{
        if(!matSet) throw std::runtime_error("Calling Tmatrix from non-set Pose object");
...
namespace NodeSearch {
    namespace DiscoverySource {
        ...
        inline const std::string EXIT_VX = "EXIT_VX";
        inline const std::string AIM_TO_GOAL = "AIM_TO_GOAL";
        inline const std::string EXPLORATION = "EXPLORATION";
    };
