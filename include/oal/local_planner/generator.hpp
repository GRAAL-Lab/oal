...
public:
    Generator(VehicleData vh_data, PruningParams pruning_params, std::shared_ptr<DebugSettings> ds = nullptr)
        : vh_data_(vh_data)
        , pruning_params_(pruning_params)
        , ds_(ds)
    {
        ...
        if (ds_ == nullptr)
            ds_ = std::make_shared<DebugSettings>();
        // Debug settings
    ...
    PathReport FindPath(const Pose& start_, Eigen::Vector2d goal_, const std::vector<ObsPtr>& obstacles_, AstarPath& path_, bool corlegs = false);
